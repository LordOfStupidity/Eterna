...
EFI_FILE* LoadFile(EFI_FILE* dir, CHAR16* path) {
    ...
    if (loadedImage == NULL) {
        gBootServices->HandleProtocol(gImageHandle, &gEfiLoadedImageProtocolGuid,
                                      (void**)&loadedImage);
    }
    static EFI_SIMPLE_FILE_SYSTEM_PROTOCOL* fileSystem;
    if (fileSystem == NULL) {
        gBootServices->HandleProtocol(loadedImage->DeviceHandle, &gEfiSimpleFileSystemProtocolGuid,
                                      (void**)&fileSystem);
    }
    if (dir == NULL) {
...
EFI_STATUS efi_main(EFI_HANDLE IH, EFI_SYSTEM_TABLE* ST) {
    ...
    if (dflt_font == NULL) {
        ...
        Print(
            ...
            dflt_font->PSF1_Header->Mode, dflt_font->PSF1_Header->CharacterSize);
    }
    ...
    gBootServices->AllocatePool(EfiLoaderData, programHeaderTableSize, (void**)&program_hdrs);
    if (program_hdrs == 0) {
    ...
    for (Elf64_Phdr* phdr = program_hdrs;
        ...
        if (phdr->p_type == PT_LOAD) {
            ...
            do {
                gBootServices->AllocatePages(AllocateAddress, MemoryType, pages, &segment);
            } while (segment == 0);
    ...
    Print(
        ...
        gop_fb->BaseAddress, gop_fb->BufferSize, gop_fb->PixelWidth, gop_fb->PixelHeight,
    ...
    void (*KernelStart)(BootInfo*)
        = ((__attribute__((sysv_abi)) void (*)(BootInfo*))elf_header.e_entry - KERNEL_VIRTUAL);
    KernelStart(&info);
