...
void free_pages(void* address, uint64_t numberOfPages) {
    ...
    dbgmsg(
        ...
        "  # of pages:  %ull\r\n"
        "  Free before: %ull\r\n",
        address, numberOfPages, TotalFreePages);
#endif  // DEBUG_PMM
    for (uint64_t i = 0; i < numberOfPages; ++i)
...
void* request_page() {
    ...
    dbgmsg(
        ...
        "\r\n",
        TotalFreePages, MaxFreePagesInARow);
#endif
...
#ifdef DEBUG_PMM
            dbgmsg(
                "  Successfully fulfilled memory request: %x\r\n"
                "\r\n",
                addr);
#endif  // DEBUG_PMM
...
void* request_pages(uint64_t numberOfPages) {
    ...
    if (numberOfPages > TotalFreePages) {
        dbgmsg(
            ...
            "Number of pages requested is larger than amount of pages "
            "available.");
        return nullptr;
    }
    ...
    if (numberOfPages > MaxFreePagesInARow) {
        dbgmsg(
            "request_pages(): \033[31mERROR\033[0m:: "
            "Number of pages requested is larger than any contiguous run of "
            "pages available.");
        return nullptr;
    }
    ...
    dbgmsg(
        ...
        "  Max run of free pages: %ull\r\n"
        "\r\n",
        numberOfPages, TotalFreePages, MaxFreePagesInARow);
#endif
    ...
    for (uint64_t i = FirstFreePage; i < PageMap.length(); ++i) {
        ...
        while (PageMap[index] == false) {
            ...
            if (index >= PageMap.length()) {
                // TODO: No memory matching criteria, should probably do a page swap from disk or something.
                panic(
                    "\033[0mRan out of memory in request_pages() "
                    ":^<\033[0m\r\n");
                return nullptr;
            }
...
constexpr uint64_t InitialPageBitmapPageCount =
    InitialPageBitmapMaxAddress / PAGE_SIZE;
constexpr uint64_t InitialPageBitmapSize = InitialPageBitmapPageCount / 8;
uint8_t InitialPageBitmap[InitialPageBitmapSize];

void init_physical(EFI_MEMORY_DESCRIPTOR* memMap, uint64_t size,
                   uint64_t entrySize) {
#ifdef DEBUG_PMM
    dbgmsg(
    ...
    /** 
     * @brief Find largest free and usable contiguous region of memory
    ...
    */
    ...
    for (uint64_t i = 0; i < entries; ++i) {
        EFI_MEMORY_DESCRIPTOR* desc =
            (EFI_MEMORY_DESCRIPTOR*)((uint64_t)memMap + (i * entrySize));
        if (desc->Type == 7) {
            if (desc->NumPages > largestFreeMemorySegmentPageCount &&
                (uint64_t)desc->PhysicalAddress + desc->NumPages <
                    InitialPageBitmapMaxAddress) {
    ...
    /**
     * @brief Use the initial pre-allocated page bitmap as a guide
    ...
    */
    ...
    for (uint64_t t = 0;
         t < TotalPages * PAGE_SIZE && t < InitialPageBitmapMaxAddress;
         t += PAGE_SIZE) {
    ...
    /**
     * @brief Calculate total number of bytes needed for a physical page
    ...
    */
    ...
    /**
     * @brief With all pages in the bitmap locked, free only the EFI conventional memory segments.
    ...
    */
    ...
    /**
     * @brief The page map itself takes up space within the largest free memory segment.
    ...
    deadSpace +=
        (uint64_t)&BLOCK_STARTING_SYMBOLS_START - (uint64_t)&READ_ONLY_DATA_END;
    ...
    uint64_t bssSize =
        reinterpret_cast<uint64_t>(&BLOCK_STARTING_SYMBOLS_END) -
    ...
    dbgmsg(
        ...
        "\r\n",
        0ULL, total_ram(), &KERNEL_PHYSICAL, TO_MiB(&KERNEL_PHYSICAL),
        &KERNEL_START, &KERNEL_END, TO_KiB(kernelSize), &TEXT_START, &TEXT_END,
        textSize, &DATA_START, &DATA_END, dataSize, &READ_ONLY_DATA_START,
        &READ_ONLY_DATA_END, rodataSize, &BLOCK_STARTING_SYMBOLS_START,
        &BLOCK_STARTING_SYMBOLS_END, bssSize, deadSpace);
}
...
void print_debug_kib() {
    dbgmsg(
        ...
        "  Used Memory: %ullKiB\r\n"
        "\r\n",
        TO_KiB(total_ram()), TO_KiB(free_ram()), TO_KiB(used_ram()));
}
...
void print_debug_mib() {
    dbgmsg(
        ...
        "  Used Memory: %ullMiB\r\n"
        "\r\n",
        TO_MiB(total_ram()), TO_MiB(free_ram()), TO_MiB(used_ram()));
}
