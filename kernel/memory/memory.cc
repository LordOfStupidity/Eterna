...
void memcpy(const void* src, const void* dest, uint64_t numBytes) {
    ...
    for (; i <= (int64_t)numBytes - 2048; i += 2048) {
        *(uint16384_t*)((uint64_t)dest + i) = *(uint16384_t*)((uint64_t)src + i);
    }
...
void volatile_read(const volatile void* ptr, volatile void* out, uint64_t length) {
    if (ptr == nullptr || out == nullptr || length == 0) return;
    ...
    if (length == 1) {
        ...
        *(uint32_t*)out = *(volatile uint64_t*)ptr;
    } else if (length == 8) {
...
void volatile_write(void* data, volatile void* ptr, uint64_t length) {
    if (data == nullptr || ptr == nullptr || length == 0) return;
    ...
    if (length == 1) {
        ...
        *(volatile uint32_t*)ptr = *(uint64_t*)data;
    } else if (length == 8) {
